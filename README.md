...
- **SSRF Protection** - Server-side URL validation with domain allowlist (`*.eagleeyenetworks.com`) prevents attacks via malicious `httpsBaseUrl` values
...
- **SSRF protection** on `httpsBaseUrl` from EEN token responses - only `*.eagleeyenetworks.com` domains are allowed, preventing attackers from redirecting API calls to internal services or cloud metadata endpoints
